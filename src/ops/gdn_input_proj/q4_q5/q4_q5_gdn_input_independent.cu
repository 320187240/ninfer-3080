...
// Registered channel triples are the full GDN geometry (4096/6144/6144) and its two-rank
// tensor-parallel N-split (2048/3072/3072); K is the shared hidden width.
template <std::int32_t kQkRows, std::int32_t kValueRows, std::int32_t kZRows>
struct GdnInputIndependent {
    static constexpr std::int32_t kValueZRows = kValueRows + kZRows;

    static void launch_q4_gemv(const Tensor& x, const Weight& weight, Tensor& out,
                               cudaStream_t stream) {
    ...
    static void launch_q4_simt(const Tensor& x, const Weight& weight, Tensor& out,
                               cudaStream_t stream) {
    ...
    static void launch_q4_simt_route(const Tensor& x, const Weight& weight, Tensor& out,
                                     cudaStream_t stream) {
    ...
    static void launch_q4(const Tensor& x, const Weight& weight, Tensor& out, cudaStream_t stream) {
        if (x.ne[1] == 1) {
    ...
    static void launch_q5_gemv(const Tensor& x, const Weight& weight, Tensor& value, Tensor& z,
                               cudaStream_t stream) {
        constexpr int kRowsPerBlock = 16;
        constexpr int kThreads      = kRowsPerBlock * 32;
        q5_rowsplit_gemv_kernel<kValueZRows, kHidden, kRowsPerBlock, 2, true, false, true,
                                kValueRows>
    ...
    template <int Cols>
    static void launch_q5_split4(const Tensor& x, const Weight& weight, Tensor& value, Tensor& z,
                                 cudaStream_t stream) {
    ...
    static void launch_q5_split4_exact(const Tensor& x, const Weight& weight, Tensor& value,
    ...
    static void launch_q5_simt_r8_c8(const Tensor& x, const Weight& weight, Tensor& value, Tensor& z,
                                     cudaStream_t stream) {
        ...
        q5_rowsplit_gemm_simt_kernel<Q5RowSplitSimtSchedule, kColsPerTile, kRowsPerBlock, kStages,
                                     true, kValueRows><<<grid, kThreads, 0, stream>>>(
            static_cast<const __nv_bfloat16*>(x.data), static_cast<const std::uint8_t*>(weight.qdata),
            static_cast<const std::uint8_t*>(weight.qhigh),
    ...
    static void launch_q5(const Tensor& x, const Weight& weight, Tensor& value, Tensor& z,
                          cudaStream_t stream) {
        if (x.ne[1] == 1) {
    ...
    static void launch_t4_pdl(const Tensor& x, const Weight& qk_weight, const Weight& value_z_weight,
        ...
        CUDA_CHECK(pdl::launch_dependent(
            ...
            q4_rowsplit_gemm_simt_kernel<Q4Schedule, true, false, 0, Q4SimtStoreEpilogue, false,
                                         true>,
            static_cast<const __nv_bfloat16*>(x.data),
    ...
    static void run(const Tensor& x, const Weight& qk_weight, const Weight& value_z_weight,
                    Tensor& qk, Tensor& value, Tensor& z, cudaStream_t stream) {
        if (x.ne[1] == 4) {
...
void q4_q5_gdn_input_independent_launch(const Tensor& x, const Weight& qk_weight,
                                        ...
                                        Tensor& z, cudaStream_t stream) {
    if (qk_weight.n == 4096) {
        GdnInputIndependent<4096, 6144, 6144>::run(x, qk_weight, value_z_weight, qk, value, z,
                                                   stream);
        return;
    }
    if (qk_weight.n == 2048) {
        GdnInputIndependent<2048, 3072, 3072>::run(x, qk_weight, value_z_weight, qk, value, z,
                                                   stream);
        return;
    }
    throw std::invalid_argument("Q4/Q5 GDN independent launch: unregistered qk row count");
}
