...
// Registered channel quadruples are the full GDN geometry (2048/2048/6144/6144) and its
// two-rank tensor-parallel N-split (1024/1024/3072/3072); K is the shared hidden width.
template <int kQueryRows, int kKeyRows, int kValueRows, int kZRows>
struct GdnConvFused {
    ...
    static GdnConvEpilogue<Publish> make_epilogue(const Tensor& conv_weight, const Tensor& conv_states,
...
#pragma unroll
    ...
    static void launch_q4_t1(const Tensor& x, const Weight& qk_weight,
    ...
    template <class Publish, bool TriggerPdl, bool JoinPdl, bool Dependent>
    static void launch_q5_t1(const Tensor& x, const Weight& value_z_weight,
                             const GdnConvEpilogue<Publish>& value_epilogue, Tensor& value,
        ...
        if constexpr (Dependent) {
            CUDA_CHECK(pdl::launch_dependent(
                ...
                q5_rowsplit_gemv_kernel<kValueZRows, kHidden, q5_rows_per_block, 2, true, false,
                                        true, kValueRows, Q5GdnDecodeEpilogue<Publish>, TriggerPdl,
                                        JoinPdl>,
                static_cast<const __nv_bfloat16*>(x.data),
                ...
                Q5GdnDecodeEpilogue<Publish>{value_epilogue,
    ...
    template <int Tokens, class Q4Schedule, class Publish, bool TriggerPdl, bool JoinPdl,
              bool Dependent>
    static void launch_q4_small_t(const Tensor& x, const Weight& qk_weight,
                                  const GdnConvEpilogue<Publish>& qk_epilogue, Tensor& query,
                                  cudaStream_t stream) {
        ...
        if constexpr (Dependent) {
            CUDA_CHECK(pdl::launch_dependent(
                ...
                q4_rowsplit_gemm_simt_kernel<Q4Schedule, false, false, 0,
                                             Q4GdnSmallTEpilogue<Tokens, Publish>, TriggerPdl,
                                             JoinPdl>,
                static_cast<const __nv_bfloat16*>(x.data),
                static_cast<const std::uint8_t*>(qk_weight.qdata),
                static_cast<const std::uint8_t*>(qk_weight.scales),
    ...
    static void launch_q5_small_t(const Tensor& x, const Weight& value_z_weight,
                                  const GdnConvEpilogue<Publish>& value_epilogue, Tensor& value,
                                  Tensor& z, cudaStream_t stream) {
        ...
        if constexpr (Dependent) {
            CUDA_CHECK(pdl::launch_dependent(
                ...
                q5_rowsplit_gemm_simt_split4_kernel<Q5RowSplitSimtSchedule, Tokens, 5, kHidden,
                                                    true, kValueRows,
    ...
    static void launch_t1(const Tensor& x, const Weight& qk_weight, const Weight& value_z_weight,
    ...
    static void launch_small_t_schedule(const Tensor& x, const Weight& qk_weight,
                                        const Weight& value_z_weight,
                                        const GdnConvEpilogue<Publish>& qk_epilogue,
                                        const GdnConvEpilogue<Publish>& value_epilogue,
                                        Tensor& query, Tensor& value, Tensor& z,
                                        cudaStream_t stream) {
        if constexpr (Order == PdlOrder::Q5ThenQ4) {
            launch_q5_small_t<Tokens, Publish, true, false, false>(x, value_z_weight, value_epilogue,
                                                                   value, z, stream);
            launch_q4_small_t<Tokens, Q4Schedule, Publish, false, true, true>(
    ...
    static void launch_small_t(const Tensor& x, const Weight& qk_weight,
                               const Weight& value_z_weight,
                               const GdnConvEpilogue<Publish>& qk_epilogue,
                               const GdnConvEpilogue<Publish>& value_epilogue, Tensor& query,
                               Tensor& value, Tensor& z, cudaStream_t stream) {
    ...
    template <PdlOrder Order, class Publish>
    static void launch_conv(const Tensor& x, const Weight& qk_weight, const Weight& value_z_weight,
        ...
        switch (x.ne[1]) {
        case 1:
            launch_t1<Order, Publish>(x, qk_weight, value_z_weight, qk_epilogue, value_epilogue,
                                      query, value, z, stream);
            break;
        case 2:
            launch_small_t<2, Order, Publish>(x, qk_weight, value_z_weight, qk_epilogue,
                                              value_epilogue, query, value, z, stream);
            break;
        case 3:
            launch_small_t<3, Order, Publish>(x, qk_weight, value_z_weight, qk_epilogue,
                                              value_epilogue, query, value, z, stream);
            break;
        case 5:
            launch_small_t<5, Order, Publish>(x, qk_weight, value_z_weight, qk_epilogue,
                                              value_epilogue, query, value, z, stream);
            break;
        case 6:
            launch_small_t<6, Order, Publish>(x, qk_weight, value_z_weight, qk_epilogue,
                                              value_epilogue, query, value, z, stream);
            break;
        default:
            throw std::invalid_argument(
                "Q4/Q5 projection-epilogue GDN conv requires T=1..3 or 5..6");
        }
        CUDA_CHECK(cudaGetLastError());
    }

    static void snapshot(const Tensor& x, const Weight& qk_weight, const Weight& value_z_weight,
                         ...
                         cudaStream_t stream) {
        const auto publish = [&]() {
            return SnapshotHistoryPublish{static_cast<__nv_bfloat16*>(conv_states.data),
    ...
    static void record(const Tensor& x, const Weight& qk_weight, const Weight& value_z_weight,
                       const Tensor& conv_weight, const Tensor& conv_states,
...
void q4_q5_gdn_input_conv_snapshot_launch(const Tensor& x, const Weight& qk_weight,
                                          ...
                                          cudaStream_t stream) {
    if (query.ne[0] == 2048) {
        GdnConvFused<2048, 2048, 6144, 6144>::snapshot(x, qk_weight, value_z_weight, conv_weight,
    ...
    if (query.ne[0] == 1024) {
        GdnConvFused<1024, 1024, 3072, 3072>::snapshot(x, qk_weight, value_z_weight, conv_weight,
                                                       conv_states, valid_columns, initial_slot,
                                                       snapshot_base_slot, query, key, value, z,
                                                       stream);
        return;
    }
    throw std::invalid_argument("Q4/Q5 GDN conv snapshot: unregistered query row count");
}
...
void q4_q5_gdn_input_conv_record_launch(const Tensor& x, const Weight& qk_weight,
                                        ...
                                        cudaStream_t stream) {
    if (query.ne[0] == 2048) {
        GdnConvFused<2048, 2048, 6144, 6144>::record(x, qk_weight, value_z_weight, conv_weight,
                                                     conv_states, valid_columns, initial_slot,
                                                     conv_record, query, key, value, z, stream);
        return;
    }
    if (query.ne[0] == 1024) {
        GdnConvFused<1024, 1024, 3072, 3072>::record(x, qk_weight, value_z_weight, conv_weight,
                                                     conv_states, valid_columns, initial_slot,
                                                     conv_record, query, key, value, z, stream);
        return;
    }
    throw std::invalid_argument("Q4/Q5 GDN conv record: unregistered query row count");
}
