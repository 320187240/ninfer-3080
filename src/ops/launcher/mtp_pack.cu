...
void mtp_split_attn_in_launch(const Tensor& attn_in, Tensor& q, Tensor& k, Tensor& gate, Tensor& v,
    ...
    mtp_split_attn_in_kernel<<<grid, kBlock, 0, stream>>>(
        ...
        static_cast<__nv_bfloat16*>(v.data), attn_in.ne[1], attn_in.ne[0],
        static_cast<std::int32_t>(q.numel() / std::max<std::int64_t>(1, attn_in.ne[1])),
        static_cast<std::int32_t>(k.numel() / std::max<std::int64_t>(1, attn_in.ne[1])));
    CUDA_CHECK(cudaGetLastError());
}
