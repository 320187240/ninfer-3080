...
// kParentRows is the fused query_key/gate_value row count and kSplitRow is the first KV row of
// that parent. The registered pairs are the full shape (7168/6144) and its two-rank N-split
// (3584/3072); K is the shared hidden width.
template <std::int32_t kParentRows, std::int32_t kSplitRow>
struct AttnInputSmallT {
    static void launch_q4_gemv(const Tensor& x, const Weight& weight, Tensor& q, Tensor& key,
                               cudaStream_t stream) {
    ...
    static void launch_q4_simt(const Tensor& x, const Weight& weight, Tensor& q, Tensor& key,
                               cudaStream_t stream) {
    ...
    static void launch_q4_simt_route(const Tensor& x, const Weight& weight, Tensor& q, Tensor& key,
                                     cudaStream_t stream) {
    ...
    static void launch_q4(const Tensor& x, const Weight& weight, Tensor& q, Tensor& key,
                          cudaStream_t stream) {
        switch (x.ne[1]) {
        case 1:
    ...
    static void launch_q5_gemv(const Tensor& x, const Weight& weight, Tensor& gate, Tensor& value,
                               cudaStream_t stream) {
    ...
    template <int Cols>
    static void launch_q5_split4(const Tensor& x, const Weight& weight, Tensor& gate, Tensor& value,
                                 cudaStream_t stream) {
    ...
    static void launch_q5_split4_exact(const Tensor& x, const Weight& weight, Tensor& gate,
    ...
    static void launch_q5_simt(const Tensor& x, const Weight& weight, Tensor& gate, Tensor& value,
                               cudaStream_t stream) {
        ...
        q5_rowsplit_gemm_simt_kernel<Q5RowSplitSimtSchedule, ColsPerTile, kRowsPerBlock, kStages,
                                     true, kSplitRow><<<grid, kThreads, 0, stream>>>(
            static_cast<const __nv_bfloat16*>(x.data), static_cast<const std::uint8_t*>(weight.qdata),
            static_cast<const std::uint8_t*>(weight.qhigh),
    ...
    static void launch_q5(const Tensor& x, const Weight& weight, Tensor& gate, Tensor& value,
                          cudaStream_t stream) {
        if (x.ne[1] == 1) {
    ...
    static void run(const Tensor& x, const Weight& query_key_weight, const Weight& gate_value_weight,
...
void q4_q5_attn_input_small_t_launch(const Tensor& x, const Weight& query_key_weight,
    ...
    if (q.ne[0] + k.ne[0] == 7168 && q.ne[0] == 6144) {
        AttnInputSmallT<7168, 6144>::run(x, query_key_weight, gate_value_weight, q, gate, k, v,
                                         stream);
        return;
    }
    if (q.ne[0] + k.ne[0] == 3584 && q.ne[0] == 3072) {
        AttnInputSmallT<3584, 3072>::run(x, query_key_weight, gate_value_weight, q, gate, k, v,
                                         stream);
        return;
    }
    throw std::invalid_argument("attention Q4/Q5 split-output parent shape is not registered");
}
