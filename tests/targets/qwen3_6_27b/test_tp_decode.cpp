// M3b end-to-end TP2 test on the real artifact: builds the Engine with tp=true over the two
// installed GPUs, prefills a short prompt and decodes greedy tokens through the two-rank
// schedule, then repeats the same request and requires identical output tokens (determinism).
// Also asserts both ranks' memory summaries are present and the TpLink transport stayed
// healthy. Skips (77) without the artifact or fewer than two CUDA devices.
...
int main() {
    ...
    if (artifact.empty() || !std::filesystem::exists(artifact) || !two_devices_available()) {
        std::cout << "skip: TP2 end-to-end test needs NINFER_TP_QWEN3_8_27B_WEIGHTS and two "
                     "CUDA devices\n";
        return 77;
    }
    ...
    try {
        ...
        const auto run = [&](const std::string& prompt, std::uint32_t max_new)
            ...
            ninfer::GenerationResult result           = engine.generate(std::move(prepared),
                                                                        std::move(request));
            // Greedy output is bounded by the request, but a natural end-of-turn stop ends the
            // round early; both runs below must still agree token-for-token.
            if (result.generated_token_ids.empty() ||
                result.generated_token_ids.size() > max_new) {
                throw std::runtime_error("TP2 generation produced " +
                                         std::to_string(result.generated_token_ids.size()) +
                                         " tokens, expected at most " + std::to_string(max_new));
            }
        ...
        const std::vector<ninfer::TokenId> first  = run(prompt, 16);
        const std::vector<ninfer::TokenId> second = run(prompt, 16);
        if (first != second) {
            throw std::runtime_error("TP2 greedy decode is not deterministic across runs");
        }

        const ninfer::MemorySummary memory = engine.memory_summary();
        if (memory.weights.used_bytes == 0 || memory.sequence.used_bytes == 0) {
            throw std::runtime_error("TP2 memory summary is empty");
        }
        std::cout << "tp2 e2e ok: " << first.size() << " greedy tokens, first="
                  << first.front() << " last=" << first.back() << "\n";
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "tp2 e2e failed: " << error.what() << "\n";
        return 1;
    }
}
